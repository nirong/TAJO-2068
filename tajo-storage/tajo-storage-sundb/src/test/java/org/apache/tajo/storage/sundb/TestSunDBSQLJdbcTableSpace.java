...
public class TestSunDBSQLJdbcTableSpace {
	  private static String jdbcUrl = SunDBTestServer.getInstance().getJdbcUrl();

	  /*
	  @Test(timeout = 1000)
	  public void testTablespaceHandler() throws Exception {
	    assertTrue((TablespaceManager.getByName("SunDB_cluster")) instanceof SunDBTablespace);
	    assertEquals("SunDB_cluster", (TablespaceManager.getByName("SunDB_cluster").getName()));

	    assertTrue((TablespaceManager.get(jdbcUrl)) instanceof SunDBTablespace);
	    assertTrue((TablespaceManager.get(jdbcUrl + "&table=tb1")) instanceof SunDBTablespace);

	    assertEquals(jdbcUrl, TablespaceManager.get(jdbcUrl).getUri().toASCIIString());
	    assertTrue(TablespaceManager.get(jdbcUrl).getMetadataProvider() instanceof SunDBMetadataProvider);
	  }

	  @Test(timeout = 1000, expected = TajoRuntimeException.class)
	  public void testCreateTable() throws IOException, TajoException {
	    Tablespace space = TablespaceManager.getByName("SunDB_cluster");
	  ...
	  public void testConnProperties() throws Exception {
	    ...
	    connProperties.put("user", "SunDB");
	    connProperties.put("password", "SunDB");

	    String uri = SunDBTestServer.getInstance().getJdbcUrl().split("\\?")[0];
	    Tablespace space = new SunDBTablespace("t1", URI.create(uri), getJsonTablespace(connProperties));
	    try {
	  ...
	  public void testConnPropertiesNegative() throws Exception {
	    Map<String, String> connProperties = new HashMap<>();
	    connProperties.put("user", "SunDBX");
	    ...
	    try {
	      space.init(new TajoConf());
	      fail("Access denied for user 'SunDBX'@'localhost' (using password: NO)");
	  ...
	  public void testTemporaryTablespace() {
	    Optional<Tablespace> ts = TablespaceManager.removeTablespaceForTest("SunDB_cluster");
