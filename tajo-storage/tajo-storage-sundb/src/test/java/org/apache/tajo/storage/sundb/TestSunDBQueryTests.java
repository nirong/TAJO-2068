...
public class TestSunDBQueryTests extends QueryTestCaseBase {
	/*	  
	  @SuppressWarnings("unused")
	  // This should be invoked for initializing SunDBTestServer
	  private static final String jdbcUrl = SunDBTestServer.getInstance().getJdbcUrl();

	  public TestSunDBQueryTests() {
	    super(SunDBTestServer.DATABASE_NAME);
	  }
	...
	//  @Test
	  ...
	  public void testCtasToHdfs() throws Exception {
	    try {
	      executeString("CREATE DATABASE mytmp").close();
	      executeString("CREATE TABLE mytmp.offload AS SELECT * FROM LINEITEM").close();

	      runSimpleTests();

	    } finally {
	      executeString("DROP TABLE IF EXISTS mytmp.offload").close();
	      executeString("DROP DATABASE IF EXISTS mytmp").close();
	    }
	  }
	  ...
	  public void testQueryWithConnProperties() throws Exception {
	    ...
	    try {
	      old = SunDBTestServer.resetAllParamsAndSetConnProperties(connProperties);
	      runSimpleTests();
	    } finally {
	      if (old.isPresent()) {
